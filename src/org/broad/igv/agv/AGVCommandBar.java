...
import org.broad.igv.agv.session.AGVHistory;
 ...
 */
public class AGVCommandBar extends javax.swing.JPanel {

    private static Logger log = Logger.getLogger(AGVCommandBar.class);
    ...
    private AGV agv;
    public static HashMap<Integer, AGVCommandBar> cmdBars = new HashMap<Integer,AGVCommandBar>();
    
    public AGVCommandBar(AGV agv) {
        this.agv = agv;
        initComponents();
        ...
        cmdBars.put(agv.getID(), this);
    }
    
    public static void destroyInstance(int id){
        cmdBars.remove(id);
        AGVZoomSliderPanel.zpanels.remove(id);///////////////////////////////////////////
    }
            ...
            public void run() {
                if (genomeListItem != null) {
                    ...
                    if (genomeListItem == GenomeListItem.ITEM_MORE) {
                        selectGenome(agv.getGenomeManager().getGenomeId());
                        //agv.loadGenomeFromServerAction();
                        return;
                    }

                    // If we haven't changed genomes we're done.
                    if (genomeListItem.getId().equalsIgnoreCase(agv.getGenomeManager().getGenomeId())) {
                        return;
                    }

                    final ProgressMonitor monitor = new ProgressMonitor();
                    final ProgressBar.ProgressDialog progressDialog = ProgressBar.showProgressDialog(agv.getMainFrame(), "Loading Genome...", monitor, false);
                    ...
                    try {
                        ...
                        genome = agv.getGenomeManager().loadAGVGenome(genomeListItem.getLocation(), null, agv);
                        ...
                        FrameManager.getDefaultFrame(agv.getID()).setChromosomeName(genome.getHomeChromosome(), true);
                        agv.doRefresh();
                        ...
                        int choice =
                                JOptionPane.showConfirmDialog(
                                        agv.getMainFrame(), "The genome file [" + genomeListItem.getLocation() +
                                                "] could not be read. Would you like to remove the selected entry?",
                                        "", JOptionPane.OK_CANCEL_OPTION);

                        if (choice == JOptionPane.OK_OPTION) {
                            agv.getGenomeManager().excludedUrl(genomeListItem.getLocation());
                            List<GenomeListItem> genomes = agv.getGenomeManager().getGenomes();
    ...
    void updateChromosomeDropdown() {
        ...
        final Genome genome = agv.getGenomeManager().getCurrentGenome();
    ...
    public void updateCurrentCoordinates() {
        ...
        if (!Globals.CHR_ALL.equals(chrName) && !FrameManager.isGeneListMode(agv.getID())) {
            p = getDefaultReferenceFrame().getFormattedLocusString();
        }
        final String position = p;
        final AGVHistory history = agv.getSession().getHistory();
    ...
    private ReferenceFrame getDefaultReferenceFrame() {
        return FrameManager.getDefaultFrame(agv.getID());
    }
...
//        roiToggleButton.setEnabled(!geneListMode);
    ...
    public Collection<String> getGenomeDisplayNames() {
        ...
        Collection<GenomeListItem> listItems = agv.getGenomeManager().getGenomes();
    ...
    public void selectGenome(String genomeId) {
        if (!getSelectableGenomeIDs().contains(genomeId)) {
            ...
            boolean found = false;
            try {
                found = agv.getGenomeManager().loadFromArchive(genomeId);
            } catch (IOException e) {
        ...
        GenomeListItem matchingItem = agv.getGenomeManager().getLoadedGenomeListItemById(genomeId);
        ...
        for (Chromosome chr : genome.getChromosomes()) {
            ...
            if (cytobands != null) {
                for (Cytoband cyto : cytobands) {
                    FeatureDB.addFeature(cyto.getLongName(), cyto, genome, agv);
                }
            }
        }
    ...
    public void refreshGenomeListComboBox() {
        ...
        String curId = agv.getGenomeManager().getGenomeId();
        Object item = agv.getGenomeManager().getLoadedGenomeListItemById(curId);
    ...
    private void initComponents() {
        ...
        backButton.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                agv.getSession().getHistory().back();
        ...
        forwardButton.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                agv.getSession().getHistory().forward();
            }
        });
        ...
        fitToWindowButton.addActionListener(new java.awt.event.ActionListener() {
            ...
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                (new FitDataToWindowMenuAction(null, 0, agv)).actionPerformed(evt);
            }
        });
        ...
        rulerLineButton.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                agv.setRulerEnabled(rulerLineButton.isSelected());
                agv.repaintDataPanels();
            }
        });
        ...
        if (showExomeButton) {
            ...
            exomeButton.setText(FrameManager.isExomeMode(agv.getID()) ? "Genome" : "Exome");
            ...
            exomeButton.addActionListener(new ActionListener() {
                ...
                public void actionPerformed(ActionEvent actionEvent) {
                    ...
                    boolean newMode = !FrameManager.isExomeMode(agv.getID());
                    if (!FrameManager.setExomeMode(newMode, showTrackMenu, agv.getID())) return;
        ...
        zoomControl = new AGVZoomSliderPanel(agv);
    ...
    private void homeButtonActionPerformed(java.awt.event.ActionEvent evt) {
        Genome genome = agv.getGenomeManager().getCurrentGenome();
        if (FrameManager.isGeneListMode(agv.getID())) {
            agv.setGeneList(null);
        }
        if (genome != null) {
    ...
    public void searchByLocus(final String searchText) {
        ...
        if ((searchText != null) && (searchText.length() > 0)) {
            String homeChr = agv.getGenomeManager().getCurrentGenome().getHomeChromosome();
            if (searchText.equalsIgnoreCase("home") || searchText.equalsIgnoreCase(homeChr)) {
                homeButtonActionPerformed(null);
            } else {
                searchTextField.setText(searchText);
                (new SearchCommand(getDefaultReferenceFrame(), searchText, agv)).execute();
            }
    ...
    private class SearchHints extends ListDataIntelliHints<String> {
        ...
        public boolean updateHints(Object context) {
            ...
            if (text.length() <= 1) {
                ...
                List<NamedFeature> features = FeatureDB.getFeaturesList(text, SearchCommand.SEARCH_LIMIT, agv);
